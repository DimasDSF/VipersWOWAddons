...
function Scrap:OnReceiveDrag()
	...
	if infoType == 'item' then
		...
		ChatFrame1:AddMessage(tostring(info[9]))
		ChatFrame1:AddMessage(tostring(info[6]).." = "..select(4, GetAuctionItemClasses()).." "..tostring(info[6] == select(4, GetAuctionItemClasses())))
		--]]
		self:UpdateButtonState()
	end
end
